...
const exec = async (opts: i18nJobOps) => {
  ...
  const langId = (uninstall || install) as Language;

  await new Promise<void>((p) =>
    installLanguage(deviceId, langId).subscribe(
      (x) => console.log(x),
      (e) => {
        console.error(e);
        p();
  ...
  //   return from((() => {debugger; return getDeviceInfo(transport)})()).pipe(
